...
top_product = top_products[0] if top_products else {'name': 'No data', 'total_revenue': 0, 'total_units': 0}
...
if hourly_revenue:
    ...
    max_revenue = max_revenue + y_padding  # Apply padding
...
# Bottom panel: Heatmap placeholder
...
d.text((45,heat_y+20), 'Weekly Revenue Pattern (Coming Soon)', font=font_kpi, fill=text)

# Placeholder message
msg = "Revenue heatmap will show daily and hourly patterns to identify optimal promotion timing"
d.text((45,heat_y+60), msg, font=font_text, fill=text)
...
# Draw sample pattern bars
for i, day in enumerate(days):
    x = base_x + (i * ((W-90) // 7))
    # Day label
    d.text((x, base_y), day, font=font_text, fill=text)
    # Activity bar (higher for weekends)
    height = 30 if day in ['Fri', 'Sat', 'Sun'] else 15
    bar_width = ((W-90) // 7) - 20
    d.rectangle([(x, base_y+25), (x+bar_width, base_y+25+height)], fill=accent)
